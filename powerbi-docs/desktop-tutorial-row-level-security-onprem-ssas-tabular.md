---
title: "Samouczek: dynamiczne zabezpieczenia na poziomie wiersza z modelem tabelarycznym usług Analysis Services w usłudze Power BI"
description: "Samouczek: dynamiczne zabezpieczenia na poziomie wiersza z modelem tabelarycznym usług Analysis Services"
services: powerbi
documentationcenter: 
author: selvarms
manager: amitaro
...
Do wykonania kroków opisanych w tym samouczku wymagana jest baza danych **AdventureworksDW2012**, którą można pobrać **[stąd.](http://msftdbprodsamples.codeplex.com/releases/view/55330)**
...
Opublikowano już wiele artykułów opisujących sposób definiowania dynamicznych zabezpieczeń na poziomie wiersza w modelu **tabelarycznym usług SQL Server Analysis Services (SSAS)**. [W naszym przykładzie będziemy postępować zgodnie z tym artykułem.](https://msdn.microsoft.com/library/hh479759.aspx) W poniższych krokach objaśniono pierwsze zadanie z tego samouczka.
...
2. Po utworzeniu i zapisaniu tabeli musimy utworzyć relację między kolumną **SalesTerritoryID** tabeli **DimUserSecurity** i kolumną **SalesTerritoryKey** tabeli **DimSalesTerritory**, jak pokazano na poniższej ilustracji. Można to zrobić w narzędziu **SSMS** przez kliknięcie prawym przyciskiem myszy tabeli **DimUserSecurity** i wybranie opcji **Edytuj**.
...
1. Po przygotowaniu magazynu danych relacyjnych należy zdefiniować model tabelaryczny. Model można utworzyć przy użyciu programu **SQL Server Data Tools (SSDT)**. Więcej informacji na temat sposobu definiowania modelu tabelarycznego [można znaleźć w tym artykule](https://msdn.microsoft.com/library/hh231689.aspx).
2. Importuj wszystkie niezbędne tabele do modelu w sposób przedstawiony poniżej.
...
7. W tej formule funkcja **LOOKUPVALUE** zwraca wszystkie wartości kolumny **DimUserSecurity[SalesTerritoryID]**, gdzie wartość **DimUserSecurity[UserName]** jest identyczna z nazwą użytkownika zalogowanego obecnie do systemu Windows, a wartość **DimUserSecurity[SalesTerritoryID]** jest identyczna z wartością **DimSalesTerritory[SalesTerritoryKey]**.
...
2. Z listy źródeł danych wybierz opcję **Baza danych usług SQL Server Analysis Services** i wybierz opcję **połącz**.
...
4. Zobaczysz, że model został wdrożony w wystąpieniu usług **Analysis Services**. Wybierz odpowiedni model i kliknij przycisk **OK**.
...
2. Wszelkie zmiany w danych po przetworzeniu modelu będą natychmiast dostępne dla użytkowników, którzy uzyskują dostęp do raportu na podstawie **połączenia na żywo** z usługi Power BI.
