...
W przypadku każdej pojemności można uzyskać pomiary dotyczące użycia w zakresie procesora CPU, pamięci i zapytania bezpośredniego. Każdy kluczowy wskaźnik wydajności ma trzy oznaczenia: **Dobre (kolor zielony)**, **Brzegowe (kolor żółty)** i **Krytyczne (kolor czerwony)**. Zalecamy monitorowanie tych metryk, aby użytkownicy mieli zapewnioną wysoką wydajność podczas korzystania z zawartości Premium.

**Monitorowanie użycia pojemności usługi Power BI Embedded odbywa się z poziomu portalu Azure.**

![](media/service-admin-premium-manage/usage-metrics-critical.png "Metryki użycia pojemności — krytyczne")

| Metryka | Opis |
| --- | --- |
| Procesor CPU |Przedstawia użycie procesora CPU rdzeni. |
| Pamięć |Reprezentuje wykorzystania pamięci rdzeni wewnętrznej bazy danych. Ta metryka w szczególności obrazuje częstotliwość usuwania modeli z pamięci na skutek braku pamięci wynikającego z używania wielu modeli. |
| Zapytania bezpośrednie |* Łączna liczba zapytań bezpośrednich i zapytań w ramach połączeń na żywo występujących w ciągu sekundy jest ograniczona.<br/>* Ograniczenia są następujące: 30/s dla jednostki P1, 60/s dla jednostki P2 i 120/s dla jednostki P3.<br/>* Liczba zapytań bezpośrednich i zapytań w ramach połączeń na żywo jest liczona po równo w ramach powyższych limitów. Na przykład, jeśli w ciągu sekundy wystąpiło 15 połączeń bezpośrednich i 15 połączeń na żywo, limit został osiągnięty.<br/>* Ma to jednakowe zastosowanie się do połączeń lokalnych i połączeń w chmurze. |

W przypadku metryk brzegowych/krytycznych użytkownicy mogą zauważyć niższą wydajność raportów i odświeżania, szczególnie w godzinach największego obciążenia.

Metryki odzwierciedlają wykorzystanie w ostatnim tygodniu i mają na celu zliczanie przeciążeń pojemności, przez które użytkownicy nie mają zapewnionej optymalnej wydajności.

Każde wystąpienie *użycia przekraczającego 80%* należy traktować jako przypadek potencjalnego obniżenia wydajności. Zbyt wiele takich przypadków oznacza, że występują poważne, odczuwalne dla użytkowników problemy z wydajnością.
